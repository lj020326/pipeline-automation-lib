...
function import_jdk_cert() {
    ...
    echo "importing pem"
#    if ${KEYTOOL} -import -trustcacerts -noprompt -keystore ${keystore} -storepass ${keystore_password} -alias ${alias} -file ${cacerts_src}/${alias}.pem >$TMP_OUT
    if ${KEYTOOL} -import -trustcacerts -noprompt -keystore ${keystore} -storepass ${keystore_password} -alias ${alias} -file ${cacerts_src}/${alias}.pem 2>&1 | tee -a $TMP_OUT
    then
...
#        ${KEYTOOL} -import -trustcacerts -noprompt -keystore ${keystore} -storepass ${keystore_password} -alias ${alias} -file ${cacerts_src}/${alias}.crt >$TMP_OUT
        ${KEYTOOL} -import -trustcacerts -noprompt -keystore ${keystore} -storepass ${keystore_password} -alias ${alias} -file ${cacerts_src}/${alias}.crt 2>&1 | tee -a $TMP_OUT
    fi
...
#    sudo security add-trusted-cert -d -r trustRoot -k "/Library/Keychains/System.keychain" "/private/tmp/securly_SHA-256.crt"
    security add-trusted-cert -d -r trustRoot -k "/Library/Keychains/System.keychain" ${cacerts_src}/${alias}.crt
