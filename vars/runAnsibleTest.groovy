...
import com.dettonville.pipeline.utils.JsonUtils // Assuming this exists for logging
    ...
    docker.image(config.dockerImage).inside("-v /var/run/docker.sock:/var/run/docker.sock --privileged") {
        ...
        if (config.testDeps) {
            ...
            for (dep in depsToInstall) {
                if (dep) {
                    log.info("${logPrefix} installing collection: ${dep}")
                    sh "ansible-galaxy collection install ${dep} --force"
                }
            }
        }
...
def call(Map params = [:]) {
    ...
    def result = runSingleAnsibleTest(config)
