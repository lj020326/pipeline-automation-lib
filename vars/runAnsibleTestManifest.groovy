            ...
            stage('Prepare Ansible Test Environment') {
                steps {
                    script {
                        ...
                        def collectionsBaseDir = "${WORKSPACE}/ansible_collections"
                        config.targetCollectionDir = "${collectionsBaseDir}/${config.collectionNamespace}/${config.collectionName}"

                        log.info("collectionsBaseDir=${collectionsBaseDir}")

                        sh """
                            echo "Current working directory: \$(pwd)"
                            echo "Creating collection directory: ${config.targetCollectionDir}"
                            mkdir -p "${config.targetCollectionDir}"
                            rsync -dar --links --exclude={.git,releases,ansible_collections} . "${config.targetCollectionDir}/"
        ...
        if (fileExists(manifestPath)) {
            ...
            if (testConfig.strategy?.matrix?.versions) {
                ...
                Map matrixDriverConfig = [
                    ...
                    // NEW: Define the executorScript as a closure
