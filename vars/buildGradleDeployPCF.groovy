            ...
            stage('Checkout') {
                steps {
                    script {
                        ...
                        env.PUBLISH_ARTIFACT = "FAILED"

                        echo "Loading pipeline configs"
                        appConfigs = loadBranchConfig(env.BRANCH_NAME)
...
def deployGreen(Map config) {
    ...
    echo "deploy ${config.pipeline.pcfAppName} to ${config.pipeline.pcfAppRouteGreen}"
