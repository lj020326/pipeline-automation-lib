...
import static com.dettonville.testing.jenkins.pipeline.StepConstants.DIR
import static com.dettonville.testing.jenkins.pipeline.StepConstants.SSH_AGENT
import static com.dettonville.testing.jenkins.pipeline.recorder.StepRecorderAssert.*
import org.junit.Before
import org.junit.Test

import static com.dettonville.testing.jenkins.pipeline.StepConstants.FILE_EXISTS
import static com.dettonville.testing.jenkins.pipeline.StepConstants.SH
...
class GitToolsMirrorSshIT extends LibraryIntegrationTestBase {
  ...
  void shouldMirrorRepositoryWithCredentialAutoLookup() {
    mockedShellCommands = [
      [
        script: "git remote -v",
        result: "origin  git@host1.domain.tld:api/pipeline-automation-lib.git (fetch)\n" +
          "origin  git@host2.domain.tld:api/pipeline-automation-lib.git (push)"
      ]
    ]
    ...
    List expectedShellCalls = [
      "git clone --mirror git@host1.domain.tld:api/pipeline-automation-lib.git pipeline-automation-lib.git",
      "git remote set-url --push origin git@host2.domain.tld:api/pipeline-automation-lib.git",
      [
        script      : "git remote -v",
        returnStdout: true
      ],
  ...
  void shouldFailWhenRemotePushOriginIsWrongTestJob() {
    mockedShellCommands = [
      [
        script: "git remote -v",
        result: "origin  git@host1.domain.tld:api/pipeline-automation-lib.git (fetch)\n" +
          "origin  git@host1.domain.tld:api/pipeline-automation-lib.git (push)"
      ]
    ]
    loadAndExecuteScript("vars/gitTools/mirrorSsh/jobs/shouldMirrorRepositoryWithCredentialAutoLookupTestJob.groovy")
  }
