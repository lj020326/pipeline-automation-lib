...
/**
 * Runs execMaven step  with default configuration by providing only the scm url, no global or local maven settings expected
 * from auto lookup here
 *
 * @return The script
 * @see vars.execMaven.ExecMavenIT
 */
def execute() {
  execMaven((SCM): [(SCM_URL): "https://subdomain.domain-new.tld/group/project1.git"])
}
