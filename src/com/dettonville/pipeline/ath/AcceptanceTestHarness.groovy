package com.dettonville.pipeline.ath
/*-
 *
...
import com.dettonville.pipeline.utils.CredentialParser
import com.dettonville.pipeline.utils.JenkinsApiUtils
import com.dettonville.pipeline.utils.ArtifactApiUtils
...
//        testResults = false
    ...
    Map initPipeline(Map params) {
        ...
        log.debug("initial pipelineConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(pipelineConfig)}")
        ...
        if (pipelineConfig.getJobCause) {
            ...
            try {
                ...
                log.info("jobCauseMap=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(jobCauseMap)}")
        ...
        log.info("final pipelineConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(pipelineConfig)}")
...
//        dsl.deleteDir()
        ...
        if (pipelineConfig.checkIfDeployJobsRan && pipelineConfig?.deployJobEnvName) {
            ...
            log.info("currentState.componentDeployJobDiffResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentState.componentDeployJobDiffResults)}")
    ...
    Map getDeployJobResults() {
        ...
        config.componentList.each { Map component ->
            ...
            log.info("componentConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentConfig)}")
...
//            componentConfig.version = buildInfo.actions.findResult { it.parameters }.findResult { it.name=="ArtifactVersion" ? it.value : null }
            ...
            if (componentConfig.hierarchicalDeployJobs) {
                ...
                if (jenkinsApiUtils.getJobArtifactFromBuildUrl(componentConfig.deployUrl, componentConfig.deployArtifactFile, componentConfig.deployResultsFile)==200) {
                    ...
                    log.debug("deployResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(deployResults)}")
        ...
        log.debug("buildResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildResults)}")
        return buildResults
    }
    ...
    List getAllBuildDiffs() {
        ...
        config.componentList.each { Map component ->
            ...
            log.debug("componentConfig.buildResultsBefore=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentConfig.buildResultsBefore)}")
            log.debug("componentConfig.buildResultsAfter=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentConfig.buildResultsAfter)}")
            ...
            log.info("[${componentConfig.name}] buildDiffs=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildDiffs)}")

            if (!buildDiffs.isEmpty()) {
                componentDiffResults.add(buildDiffs)
            }
        }
        log.info("componentDiffResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentDiffResults)}")
...
//        Boolean isDiff = jsonUtils.isJsonDiff(buildResultsBefore, buildResultsAfter, true)
        ...
        log.info("config=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(config)}")
        log.info("buildResultsBefore=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildResultsBefore)}")
        log.info("buildResultsAfter=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildResultsAfter)}")
...
//        log.debug("diffs=${diffs}")
        ...
        log.info("deployJobDiffs=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(deployJobDiffs)}")
    ...
    Map loadPipelineConfig(Map params, String configFile=null) {
        ...
        log.debug("config(1)=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(config)}")

        // handle config yml
        // handle config file
        if (configFile != null && dsl.fileExists(configFile)) {
            Map configSettings = dsl.readYaml file: "${configFile}"
            config = com.dettonville.pipeline.utils.MapMerge.merge(config, configSettings.pipeline)
        ...
        if (params?.yml) {
            ...
            log.debug("ymlConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(ymlConfig)}")
            config = com.dettonville.pipeline.utils.MapMerge.merge(config, ymlConfig.pipeline)
        }

        log.debug("params=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(params)}")
...
//            }
//        }
        config = com.dettonville.pipeline.utils.MapMerge.merge(config, params)
        log.debug("config(2)=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(config)}")
...
//    config.browserstackName = jobParts.collect{ it.toLowerCase().replaceAll(' ', '').replaceAll('-', '').replaceAll('_', '') }.join('-')
        ...
        if (config.debugPipeline) {
            log.setLevel(com.dettonville.pipeline.utils.logging.LogLevel.DEBUG)
            log.debug("**********************")
        ...
        log.debug("final config=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(config)}")
    ...
    List getCredentialIdList(Map config) {
        ...
        dsl.dir(config.checkoutDir) {
            ...
            if (envConfigs.size()>0) {
                log.debug("envConfigs=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(envConfigs)}")
            }
...
//    dsl.sh "find ${envFilePath} -type f -printf \"%TY-%Tm-%Td %TH:%TM:%.2Ts %m:%u:%g %p\\n\" | sort -k 3,3"
    ...
    Map runTests(Map config=pipelineConfig.clone()) {
        log.info("started")
        log.debug("config=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(config)}")
        ...
        if (config.useTestGroups && config?.testGroups) {
            ...
            config.testGroups.eachWithIndex { it, i ->
                ...
                log.info("i=${i} it=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(it)}")
                ...
                log.debug("i=${i} groupConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(groupConfig)}")
        ...
        log.info("currentState=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentState.findAll { it.key != 'componentDeployJobSnapshots' })}")
    ...
    boolean runTestCases(Map config) {
        ...
        config.testCases.eachWithIndex { it, i ->
            ...
            log.debug("i=${i} testConfig=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(testConfig)}")
...
//        String logPrefix="[${config.testCaseLabel}-${config.nodeId}] runAcceptanceTest():"
        ...
        currentRunState.duration = com.dettonville.pipeline.utils.Utilities.getDurationString(milliseconds)
...
//            currentState.nodes[dsl.env.NODE_NAME].bsLocalAgents[config.browserstackLocalIdentifier].runs[config.testCaseLabel] = currentRunState
        ...
        log.info("currentRunState=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentRunState)}")
...
//            log.debug("initializing currentState.nodes[${dsl.env.NODE_NAME}].bsLocalAgents[${config.browserstackLocalIdentifier}] map")
        ...
        log.debug("started with currentAgentState[node=[${dsl.env.NODE_NAME}], browserstackLocalIdentifier=${config.browserstackLocalIdentifier}]=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentAgentState)}")
...
//        log.info("starting -> currentState.nodes[${dsl.env.NODE_NAME}].bsLocalAgents[${config.browserstackLocalIdentifier}]=${currentState.nodes[dsl.env.NODE_NAME].bsLocalAgents[config.browserstackLocalIdentifier]}")
            log.debug("started AGENT -> currentState.nodes=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentState.nodes)}")
...
//        dsl.sh "cat ${config.bsLogFile}"
            ...
            currentState.nodes[dsl.env.NODE_NAME].bsLocalAgents[config.browserstackLocalIdentifier].runningRunCount -= 1
            log.debug("finished -> currentState=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentState)}")
...
//        log.debug("started -> currentState=${JsonUtils.printToJsonString(currentState)}")
        log.debug("started -> currentState.nodes=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentState.nodes)}")
...
//    Integer priorBuildNumber = priorBuildInfo.number
        ...
        log.debug("currentTestResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(currentTestResults)}")
...
//    if (getJobArtifact(testResultsFile, priorBuildNumber)==200) {
        if (jenkinsApiUtils.getJobArtifact(testResultsFile)==200) {
            ...
            log.debug("testResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(testResults)}")
        } else {
        ...
        if (testResults.history.size()>config.maxTestResultsHistory) {
            ...
            log.debug("truncated testResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(testResults)}")
        }
