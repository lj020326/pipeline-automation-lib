...
//            emailBody += " <a href=\"${artifactUrl}\">[${fileVersion}]</a>"
    ...
    String getDeployVersionAndCauseLinks(Map config, Map buildResults) {
        ...
        if (buildResults?.deployResults) {
            log.debug("buildResults.deployResults=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildResults.deployResults)}")
            fileVersion = buildResults.deployResults.fileVersion
            artifactUrl = buildResults.deployResults.artifactUrl
        } else if (config.getLatestArtifactVersion) {
            log.debug("buildResults.latestArtifactVersionInfo=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(buildResults.latestArtifactVersionInfo)}")
...
//            componentBuildVersionAndCauseLink += " (${componentBuildCause})"
    ...
    String getDeployJobRunInfo(Map config, Map currentState, String styleColor) {
        ...
        currentState.componentDeployJobDiffResults.each { Map component ->
            ...
            log.debug("componentBuildCauseBefore=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentBuildCauseBefore)}")
            log.debug("componentBuildCauseAfter=${com.dettonville.pipeline.utils.JsonUtils.printToJsonString(componentBuildCauseAfter)}")
